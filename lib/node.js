...
exports.Node = Binding.extend({
	updateNodes: function() {
		...
		if (parentNode != null) {
			...
			// the next node might not be in the DOM yet, so we just ignore it
			if (beforeNode != null && beforeNode.parentNode !== parentNode) beforeNode = null;

			// place in the DOM, but only if not already there
			if (!util.isNodeAtDOMPosition(this.node, parentNode, beforeNode)) this.insertNode(parentNode, beforeNode);
		}
