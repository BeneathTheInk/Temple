...
exports.Node = Binding.extend({
	updateNodes: function() {
		...
		if (parentNode != null && (
			...
			parentNode === this.node.parentNode &&
			nextSiblingNode !== this.node.nextSibling ))) {
			parentNode.insertBefore(this.node, nextSiblingNode);
		}
...
exports.Element = Node.extend({
	constructor: function(tagname) {
		...
		Node.apply(this, _.toArray(arguments).slice(1));
	},
	...
	attr: function(name, value) {
		...
		if (_.isUndefined(value)) return this.getAttribute(name);
	...
	prop: function(name, value) {
		if (_.isObject(name) && value == null) {
			_.each(name, function(v, n) { this.prop(n, v); }, this);
			return this;
		}

		if (_.isUndefined(value)) return this.node[name];
	...
	style: function(name, value) {
		if (_.isObject(name) && value == null) {
			_.each(name, function(v, n) { this.style(n, v); }, this);
			return this;
		}

		if (_.isUndefined(value)) return getComputedStyle(this.node)[name];
	...
	addClass: function() {
		_.flatten(_.toArray(arguments)).forEach(function(className) {
