...
module.exports = Binding.extend({
	...
	updateRow: function(key, scope) {
		...
		var nscope = scope.newScopeFromPath(util.joinPathParts(this.path, key));
	...
	dependOn: function(scope) {
		...
		var onChange = (function(s) {
			var parts = util.splitPath(s.path),
				keys = _.keys(this.rows);
			
			if (parts.length === 1) {
				this.render(scope);
			} else if (s.type === "delete") {
				this.removeRow(parts[1]);
			} else {
				this.updateRow(parts[1], scope);
			}
		}).bind(this);
	...
	render: function(scope) {
		...
		// remove all rows except for what's in keys
