...
module.exports = util.subclass.call(EventEmitter, {
	...
	// creates/caches/retrieves a child model from a value at local path
	_spawn: function(path) {
		...
		this.children[path] = child = new (this.constructor)(val);
	...
	getModel: function(parts) {
		...
		while (parts.length) {
			model = model._spawn(parts.shift());
		}
	...
	set: function(parts, value, options) {
		...
		if (!parts.length) {
			...
			// try merge or reset
			if (reset || this.handle("isLeaf") || this.handle("merge", value) === false) {
	...
	notify: function(path, oval, options) {
		...
		// dirty check that the values are indeed different
		nval = this.value;
		if (nval === oval) return;
	...
	_findAllMatchingPaths: function(model, value, parts, paths, base) {
		...
		if (_.isRegExp(part)) {
			handle("keys").forEach(function(k) {
				this._findAllMatchingPaths(model, handle("get", k), rest, paths, base.concat(k));
			}, this);
		} else if (part === "**") {
			if (handle("isLeaf")) {
			...
			handle("keys").forEach(function(k) {
				...
				this._findAllMatchingPaths(model, handle("get", k), _rest, paths, _base);
			}, this);
		} else {
			this._findAllMatchingPaths(model, handle("get", part), rest, paths, base.concat(part));
		}
