...
module.exports = util.subclass.call(EventEmitter, {
	...
	get: function(parts) {
		var val, stayLocal = false, fallbacks, fb;
		parts = util.splitPath(parts);
		...
		if (_.isUndefined(val) && !stayLocal) {
			fallbacks = [].concat(this._fallbacks);

			while (fallbacks.length) {
				fb = fallbacks.shift();
				val = fb.getScope(parts).value;
				if (Deps.active) fb.depend(parts);
				if (val != null) break;
				fallbacks = fallbacks.concat(fb._fallbacks);
			}
		}
	...
	_findAllMatchingPaths: function(scope, value, parts, paths, base) {
		...
		if (_.isRegExp(part)) {
			handle("keys").forEach(function(k) {
				this._findAllMatchingPaths(scope, handle("get", k), rest, paths, base.concat(k));
			}, this);
		} else if (part === "**") {
			handle("keys").forEach(function(k) {
				isEdge = false;
				...
				this._findAllMatchingPaths(scope, handle("get", k), _rest, paths, _base);
			}, this);

			// edges are scopes without keys
