...
module.exports = util.subclass.call(EventEmitter, {
	...
	clone: function() {
		var clone = new Scope(this.value);
		clone._handlers = _.clone(this._handlers);
		clone._fallbacks = _.clone(this._fallbacks);
		return clone;
	},
	...
	handle: function(handler) {
		if (_.isObject(handler)) {
			...
			var h = this._handler(this.value),
				args = _.toArray(arguments).slice(1);

			args.unshift(this.value);
			return h[handler].apply(this, args);
		}
	},
	...
	set: function(parts, value, options) {
		...
		var remove = options.remove,
			...
			reset = options.reset,
			children_options = _.extend({}, options, { silent: true, reset: true });
		...
		if (!parts.length) {
			...
			if (reset || this.handle("merge", value) === false) {
				...
				_.each(this.children, function(c, p) {
					c.set([], this.handle("get", p), children_options);
				}, this);
		...
		// single path is a basic set
			...
			if (remove || reset || child.handle("merge", value) === false) {
				...
				if (remove || this.handle("set", path, value) !== false) {
					...
					if (child.value !== nval) child.set([], nval, children_options);

					if (nval !== oval && !silent) this.emit("change", {
						scope: this,
	...
	// sets the value at path to undefined
	...
	forEach: function(fn, ctx) {
		this.handle("enumerate", fn, ctx);
		return this;
	},

	keys: function() { return this.handle("keys"); },
	has: function(path) { return this.handle("hasOwn", path); },
