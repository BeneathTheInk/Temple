...
// creates a focused handle function from scope and value
exports.createHandle = function(scope, val) {
	var handler = scope._handler(val);

	return function(m) {
		var args = _.toArray(arguments).slice(1);
		args.unshift(val);
		return handler[m].apply(scope, args);
	}
}
...
// parses a complex tag string
exports.parseTag = function(str) {
	return require("./parse/tag-parser.pegjs").parse(str);
}
