...
exports.forEach = function(obj, fn) {
	if (Adaptor.isAdaptor(obj)) return obj.enumerate(fn);
	else _.each(obj, fn);
}
...
exports.findAllMatchingPaths = function(obj, parts, paths, base) {
	...
	if (_.isRegExp(part)) {
		forEach(obj, function(val, k) {
			if (part.test(k)) findAllMatchingPaths(val, rest, paths, base.concat(k));
		});
	} else if (part === "**") {
		...
		forEach(obj, function(val, k) {
			...
			findAllMatchingPaths(val, _rest, paths, _base);
		});
	} else {
