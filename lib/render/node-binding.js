...
NodeBinding[NODE_TYPE.ELEMENT] = NodeBinding.extend({
	...
	render: function() {
		...
		// attributes
		this.attributes = Binding.buildDOM(this.template.attributes, this.temple);
		this.attributes.forEach(function(attr) { attr.render(el); });
...
NodeBinding[NODE_TYPE.ATTRIBUTE] = NodeBinding.extend({
	
	initialize: function() {},

	parseTextArgument: function(template) {
		...
		var children = template.map(function(t) {
		...
		var bindings =
		update.bindings = Binding.buildText(children, this.temple);

		return update;
		
		function update(scope) {
			bindings.forEach(function(b) { b.update(scope); });
			return _.pluck(bindings, "value").join("");
		}
	},

	parseTagArgument: function(template) {
		var raw = "",
			d = TAG_DELIMITERS,
			val, toUpdate;

		template.forEach(function(child, index) {
			switch(child.type) {
				case NODE_TYPE.TEXT:
					raw += child.value;
					break;
		...
		function walk(v, k, l) {
			...
			if (_.isString(v)) {
				...
				if (s > -1) {
					...
					if (!isNaN(index) && template[index] != null) {
	...
	render: function(el) {
		...
		var decorators = this.temple._decorators[this.name],
			children = this.template.children;
		
		if (!_.isArray(decorators) || !decorators.length) {
			this.isDecorated = false;
			this.textValue = this.parseTextArgument(children);
		} else {
			this.isDecorated = true;
			this.decorators = decorators.map(function(fn) {
				var d = fn.call(this.temple, el, children);
				if (!_.isObject(d)) return;
				
				if (d.parse == null) d.parse = "tag";
				if (this.textValue == null && d.parse === "text") {
					this.textValue = this.parseTextArgument(children);
				} else if (this.tagValue == null && d.parse === "tag") {
					this.tagValue = this.parseTagArgument(children);
				}
	...
	_update: function(scope) {
		if (!this.isDecorated) {
			this.value = this.textValue(scope);
			if (this.node instanceof window.Element) {
				this.node.setAttribute(this.name, this.value);
			}
		} else {
			this.decorators.forEach(function(d) {
				if (_.isFunction(d.update)) {
					var args = [];
					if (d.parse === "tag") args = this.tagValue(scope);
					else if (d.parse === "text") args = [ this.textValue(scope) ];
					d.update.apply(scope, args);
				}
			}, this);
		}
	},

	appendTo: function(parent, before) { return; },

	destroy: function() {
		if (this.isDecorated) {
			this.decorators.forEach(function(d) {
				if (_.isFunction(d.destroy)) d.destroy.call(this.temple);
			}, this);
			delete this.decorators;
		}

		if (this.textValue) this.textValue.bindings.forEach(function(b) { b.destroy(); });
	}
});
