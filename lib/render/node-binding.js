...
NodeBinding[NODE_TYPE.ELEMENT] = NodeBinding.extend({
	...
	parseTextArgument: function(template) {
		...
		var bindings = Binding.buildText(children, this.temple);
	...
	render: function() {
		...
		this.template.attributes.forEach(function(attr) {
			...
			if (!_.isArray(decorators) || !decorators.length) {
				attribute.value = this.parseTextArgument(attr.children);
			} else {
				attribute.decorators = decorators.map(function(fn) {
					var d = fn.call(this.temple, el, attr.children);
					if (!_.isObject(d)) return;
					
					if (d.parse == null) d.parse = "tag";
					if (attribute.textValue == null && d.parse === "text") {
						attribute.textValue = this.parseTextArgument(attr.children);
					} else if (attribute.tagValue == null && d.parse === "tag") {
						attribute.tagValue = this.parseTagArgument(attr.children);
					}
	...
	_update: function(scope) {
		...
		this.attributes.forEach(function(attr) {
			...
			attr.comp = Deps.autorun(function(comp) {
				if (attr.value) {
					self.node.setAttribute(attr.name, attr.value(scope));
				} else {
					attr.decorators.forEach(function(d) {
						if (_.isFunction(d.update)) {
							var args = [];
							if (d.parse === "tag") args = attr.tagValue(scope);
							else if (d.parse === "text") args = [ attr.textValue(scope) ];
