...
Section.prototype.spawnScope = function(val, scope) {
	var nscope = scope.spawn(val),
		self = this,
		closeScope;
...
Section.prototype.process = function(scope) {
	...
	if (Section.isEmpty(val)) {
		if (this.inverted) {
			this.update(0, this.spawnScope(val, scope));
			this.remove(1);
		} else {
			this.remove(0);
		}
		...
		if (this.inverted) {
			...
			if (_.isArray(val)) {
				val.forEach(function(v, index) {
					var nscope = this.spawnScope(v, scope);
