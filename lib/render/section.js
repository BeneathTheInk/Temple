...
Section.prototype.spawnScope = function(path, scope) {
	...
	// but listen to child and vice versa
	child.on("change", onChange);
	nscope.on("change", onChange);
	...
	function onChange(chg) {
		if (setting) return;
		setting = true;
		var cscope = this === nscope ? child : nscope;
		cscope.notify(chg.keypath, chg.value, chg.oldValue);
		setting = false;
	}

	function disconnect() {
		child.removeListener("change", onChange);
		nscope.removeListener("change", onChange);
...
Section.prototype.destroy = function() {
	...
	this._scopes.forEach(function(close) { close(); });
	this._scopes = [];
