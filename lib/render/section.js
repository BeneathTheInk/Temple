...
Section.prototype.update = function(index, ctx) {
	var row = this.rows[index];
	if (row == null) row = this.rows[index] = this.build(this.template.children);
	row.forEach(function(row) { row.update(ctx); });
	return this;
}
...
Section.prototype.process = function(ctx) {
	var val = ctx.get(this.template.value);
	
	if (Section.isEmpty(val)) {
		if (this.inverted) {
			this.update(0, ctx.spawn(val));
			this.destroy(1);
		} else {
			this.destroy(0);
		}
